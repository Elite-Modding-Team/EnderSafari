...
public class ModelFairy extends ModelBase {
    ...
    @Override
    public void render(@NotNull Entity entity, float f, float f1, float age, float f3, float f4, float f5) {
        OpenGlHelper.setLightmapTextureCoords(OpenGlHelper.lightmapTexUnit, 240f, 240f);
        GlStateManager.pushMatrix();

        double sin = Math.sin(Math.toRadians(age % 360) * 24F);
        wingL1.rotateAngleY = 0.3926991F + Math.abs((float) Math.toRadians(45f * (float) sin));
        wingL2.rotateAngleY = 0.3926991F + Math.abs((float) Math.toRadians(45f * (float) sin));
        wingR1.rotateAngleY = -(0.3926991F + Math.abs((float) Math.toRadians(45f * (float) sin)));
        wingR2.rotateAngleY = -(0.3926991F + Math.abs((float) Math.toRadians(45f * (float) sin)));
        head.render(f5);
        chest.render(f5);
        armR.render(f5);
        armL.render(f5);
