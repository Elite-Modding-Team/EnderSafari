...
public class FlyNodeProcessor extends WalkNodeProcessor {
    ...
    public @Nonnull PathPoint getStart() {
        ...
        return openPoint(MathHelper.floor(entityIn.getEntityBoundingBox().minX), MathHelper.floor(entityIn.getEntityBoundingBox().minY + 0.5D), MathHelper.floor(entityIn.getEntityBoundingBox().minZ));
    }
    ...
    public int findPathOptions(@Nonnull PathPoint[] pathOptions, @Nonnull PathPoint currentPoint, @Nonnull PathPoint targetPoint, float maxDistance) {
        ...
        for (EnumFacing enumfacing : EnumFacing.values()) {
            PathPoint pathpoint = getSafePoint(entityIn, currentPoint.x + enumfacing.getXOffset(), currentPoint.y + enumfacing.getYOffset(), currentPoint.z + enumfacing.getZOffset());
    ...
    private PathPoint getSafePoint(Entity entity, int x, int y, int z) {
        boolean i = entityFits(entity, x, y, z);
        return i ? openPoint(x, y, z) : null;
    }

    private boolean entityFits(Entity entity, int x, int y, int z) {
            ...
            for (int j = y; j < y + entitySizeY; ++j) {
                for (int k = z; k < z + entitySizeZ; ++k) {
                    ...
                    if (bs.getMaterial() != Material.AIR) {
                        AxisAlignedBB bb = bs.getCollisionBoundingBox(entity.world, mutableblockpos);
                        if (bb != null) {
                            return false;
                        }
                    }
