...
public class EntityUtil {
    public static boolean isHardDifficulty(@Nonnull World world) {
    ...
    public static void cancelCurrentTasks(@Nonnull EntityLiving entity) {
    ...
    public static @Nonnull IAttributeInstance removeModifier(@Nonnull EntityLivingBase entity, @Nonnull IAttribute p, @Nonnull UUID u) {
        IAttributeInstance att = entity.getEntityAttribute(p);
    ...
    public static @Nullable BlockPos findClearLandingSurface(@Nonnull EntityLiving entity, int x, int z, int minY, int maxY) {
        double origX = entity.posX;
        double origY = entity.posY;
        double origZ = entity.posZ;
        int y = maxY;
        boolean canLand = canLandAtLocation(entity, x, y, z);
        while (!canLand) {
            --y;
            if (y < minY) {
                break;
            }
            canLand = canLandAtLocation(entity, x, y, z);
        }
    ...
    private static boolean canLandAtLocation(@Nonnull EntityLiving entity, int x, int y, int z) {
        World world = entity.getEntityWorld();
        entity.setPosition(x + 0.5, y, z + 0.5);
        if (!SpawnUtil.isSpaceAvailableForSpawn(world, entity, false, false)) {
            return false;
        }
