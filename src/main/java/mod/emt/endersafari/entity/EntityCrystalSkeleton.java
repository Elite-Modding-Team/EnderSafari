...
import mod.emt.endersafari.entity.navigator.PathNavigateGroundMounted;
...
// TODO: Crystal horse mount? Some other stuff to make it unique
public class EntityCrystalSkeleton extends EntitySkeleton implements IRangedAttackMob {
    ...
    private final EntityAIAttackRangedBow<EntityCrystalSkeleton> aiArrowAttack = new EntityAIAttackRangedBow<>(this, 1.0D, 20, 15.0F);
    private final EntityAIAttackMelee aiAttackOnCollide = new EntityAIAttackMelee(this, 1.25D, false) {
        @Override
        public void resetTask() {
            super.resetTask();
            EntityCrystalSkeleton.this.setSwingingArms(false);
        }

        @Override
        public void startExecuting() {
            super.startExecuting();
            EntityCrystalSkeleton.this.setSwingingArms(true);
        }
    };
    ...
    public EntityCrystalSkeleton(World world) {
        super(world);
    ...
    protected void applyEntityAttributes() {
        ...
        this.getEntityAttribute(SharedMonsterAttributes.ARMOR).setBaseValue(ESConfig.ENTITIES.FALLEN_KNIGHT.armor);
        this.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).setBaseValue(ESConfig.ENTITIES.FALLEN_KNIGHT.attackDamage);
        this.getEntityAttribute(SharedMonsterAttributes.FOLLOW_RANGE).setBaseValue(32.0D);
        this.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(ESConfig.ENTITIES.FALLEN_KNIGHT.maxHealth);
        this.getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).setBaseValue(ESConfig.ENTITIES.FALLEN_KNIGHT.movementSpeed);
    }
    ...
    private void addRandomArmor() {
        ...
        if (this.rand.nextFloat() >= 0.40F) {
            ...
            this.setItemStackToSlot(EntityEquipmentSlot.MAINHAND, new ItemStack(ModItemsES.GUARDIAN_BOW));
        }
    }
    ...
    public IEntityLivingData onInitialSpawn(@NotNull DifficultyInstance diff, @Nullable IEntityLivingData livingData) {
        ...
        /*if (!this.world.isRemote) {
            if (this.rand.nextFloat() <= ESConfig.ENTITIES.FALLEN_KNIGHT.mountChance && this.world.canSeeSky(new BlockPos(this))) {
                this.spawnSteed();
            }
        }*/
    ...
    protected @NotNull SoundEvent getHurtSound(@NotNull DamageSource source) {
        return ModSoundEventsES.ENTITY_CRYSTAL_SKELETON_HURT.getSoundEvent();
    }

    @Override
    protected @NotNull SoundEvent getDeathSound() {
        return ModSoundEventsES.ENTITY_CRYSTAL_SKELETON_DEATH.getSoundEvent();
    }

    @Override
    protected void playStepSound(@NotNull BlockPos pos, @NotNull Block block) {
        this.playSound(ModSoundEventsES.ENTITY_CRYSTAL_SKELETON_STEP.getSoundEvent(), 0.15F, 1.5F);
    }

    @Nullable
    @Override
    protected ResourceLocation getLootTable() {
        return ModLootTablesES.CRYSTAL_SKELETON;
    }
}