...
import mod.emt.endersafari.entity.EntityConcussionChargePrimed;
...
public class ESBlockConcussionCharge extends BlockTNT {
    public ESBlockConcussionCharge(String name) {
        super();
    ...
    public void onExplosionDestroy(World world, @NotNull BlockPos pos, @NotNull Explosion explosion) {
        if (!world.isRemote) {
            EntityChargePrimed entityPrimed = new EntityConcussionChargePrimed(world, ((float) pos.getX() + 0.5F), pos.getY(), ((float) pos.getZ() + 0.5F), explosion.getExplosivePlacedBy());
    ...
    public void explode(World world, @NotNull BlockPos pos, @NotNull IBlockState state, @NotNull EntityLivingBase igniter) {
        if (!world.isRemote) {
            if (state.getValue(EXPLODE)) {
                EntityChargePrimed entityPrimed = new EntityConcussionChargePrimed(world, ((float) pos.getX() + 0.5F), pos.getY(), ((float) pos.getZ() + 0.5F), igniter);
                world.spawnEntity(entityPrimed);
    ...
    public void addInformation(@NotNull ItemStack stack, @Nullable World world, @NotNull List<String> list, @NotNull ITooltipFlag flag) {
        list.add(TextFormatting.GRAY + net.minecraft.util.text.translation.I18n.translateToLocal("tile." + Objects.requireNonNull(this.getRegistryName()) + ".tooltip"));
