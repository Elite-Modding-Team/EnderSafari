...
import mod.emt.enderzoo.entity.IEZFlyingMob;
...
public class FlyingMoveHelper extends EntityMoveHelper {
    private final @Nonnull IEZFlyingMob mob;
    private final double maxDescentSpeed = 0.1;

    public FlyingMoveHelper(@Nonnull IEZFlyingMob flier) {
        super(flier.asEntityCreature());
        this.mob = flier;
    }
    ...
    public void onUpdateMoveHelper() {
        if (!entity.getNavigator().noPath()) {
            ...
            double lookX = lookHelper.getLookPosX();
            double lookY = lookHelper.getLookPosY();
            double lookZ = lookHelper.getLookPosZ();
